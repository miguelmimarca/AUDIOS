    ...
    for(int i = 0; i < totalOrders; i++)
    {
        if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
        {
            ...
            ArrayResize(orders, ordersCount + 1);
                ...
                orders[ordersCount].isPending = (OrderType() == OP_BUYLIMIT || OrderType() == OP_SELLLIMIT ||
...
// DEBUG: Intentando cerrar orden
Print("DEBUG CIERRE => Ticket=", OrderTicket(), " Symbol=", OrderSymbol(), " Precio=", Bid, " SL=", OrderStopLoss(), " TP=", OrderTakeProfit());
...
int GetSymbolDigits(string symbol)
{
    ...
    return digits < 0 ? _Digits : digits;
}

string FormatPrice(string symbol, double value)
{
    if(value == 0)
        return "";
    return DoubleToString(value, GetSymbolDigits(symbol));
}
...
void CleanupOrderUI(int &tickets[], int total)
{
...
void UpdatePanelInfo()
{
    ...
    for(int i = 0; i < ordersCount; i++)
    {
        ...
        int activeCount = ArraySize(activeTickets);
        ArrayResize(activeTickets, activeCount + 1);
        activeTickets[activeCount] = orders[i].ticket;
        ...
        EnsureReadOnlyEdit(entryName, colX[1], rowY, colWidths[1], FormatPrice(orders[i].symbol, orders[i].entryPrice));

        string slbName = "GO_SLB_" + IntegerToString(orders[i].ticket);
        EnsureReadOnlyEdit(slbName, colX[2], rowY, colWidths[2], FormatPrice(orders[i].symbol, orders[i].localSL));

        string tpbName = "GO_TPB_" + IntegerToString(orders[i].ticket);
        EnsureReadOnlyEdit(tpbName, colX[3], rowY, colWidths[3], FormatPrice(orders[i].symbol, orders[i].localTP));
        ...
        EnsureEdit(slvName, colX[4], rowY, colWidths[4], slActive ? FormatPrice(orders[i].symbol, orders[i].localSL) : "");

        string tpvName = "GO_TPV_" + IntegerToString(orders[i].ticket);
        EnsureEdit(tpvName, colX[5], rowY, colWidths[5], tpActive ? FormatPrice(orders[i].symbol, orders[i].localTP) : "");

        string pl1Name = "GO_PL1_" + IntegerToString(orders[i].ticket);
        EnsureEdit(pl1Name, colX[6], rowY, colWidths[6], p1Active ? FormatPrice(orders[i].symbol, orders[i].partialLevel1) : "");
        ...
        EnsureEdit(pl2Name, colX[8], rowY, colWidths[8], p2Active ? FormatPrice(orders[i].symbol, orders[i].partialLevel2) : "");
        ...
        EnsureEdit(beEditName, colX[10], rowY, colWidths[10], beLineActive ? FormatPrice(orders[i].symbol, orders[i].breakevenLevel) : "");
    ...
    if(row < 19)
    {
        EnsureLabel("GO_PENDING_TITLE", panelStartX, rowY, "PENDIENTES");
        rowY += rowHeight;
        row++;
    }

    for(int i = 0; i < ordersCount && row < 20; i++)
    {
        if(!OrderSelect(orders[i].ticket, SELECT_BY_TICKET, MODE_TRADES))
            continue;
        if(!orders[i].isPending)
        ...
        string typeStr = (orders[i].type == OP_BUY) ? "BUY" : "SELL";
        string labelName = "GO_LABEL_" + IntegerToString(orders[i].ticket);
        string labelText = IntegerToString(orders[i].ticket) + " " + orders[i].symbol + " " + typeStr + " (PEND)";
        EnsureReadOnlyEdit(labelName, colX[0], rowY, colWidths[0], labelText);
        ...
        string slvName = "GO_SLV_" + IntegerToString(orders[i].ticket);
        EnsureEdit(slvName, colX[4], rowY, colWidths[4], "");
        string tpvName = "GO_TPV_" + IntegerToString(orders[i].ticket);
        EnsureEdit(tpvName, colX[5], rowY, colWidths[5], "");
        string pl1Name = "GO_PL1_" + IntegerToString(orders[i].ticket);
        EnsureEdit(pl1Name, colX[6], rowY, colWidths[6], "");
        string pl1pctName = "GO_PL1PCT_" + IntegerToString(orders[i].ticket);
        EnsureEdit(pl1pctName, colX[7], rowY, colWidths[7], "");
        string pl2Name = "GO_PL2_" + IntegerToString(orders[i].ticket);
        EnsureEdit(pl2Name, colX[8], rowY, colWidths[8], "");
        string pl2pctName = "GO_PL2PCT_" + IntegerToString(orders[i].ticket);
        EnsureEdit(pl2pctName, colX[9], rowY, colWidths[9], "");
        string beEditName = "GO_BEL_" + IntegerToString(orders[i].ticket);
        EnsureEdit(beEditName, colX[10], rowY, colWidths[10], "");
        string beBtnName = "GO_BE_" + IntegerToString(orders[i].ticket);
        EnsureButton(beBtnName, colX[11], rowY - 2, 36, 16, "BE+", false);
    ...
    CleanupOrderUI(activeTickets, ArraySize(activeTickets));
}
