...
void CreateLinesForOrders()
{
    ...
    CleanupStaleObjects(tickets, ordersFound);
}
...
void UpdateGlobalsFromLine(string lineName)
{
    ...
    if(suffix == "SL" || suffix == "TP" || suffix == "PL1" || suffix == "PL2" || suffix == "BE")
        UpdateLineLabel(ticket, suffix, price, GetOrderColor(ticket));
}
...
void ToggleAction(int ticket, string lineSuffix, string activeSuffix)
{
    ...
    UpdateLineLabel(ticket, lineSuffix == "BE" ? "BE" : lineSuffix, basePrice, orderColor);
...
void ApplyDirectBreakeven(int ticket)
{
    ...
    UpdateLineLabel(ticket, "SL", entry, GetOrderColor(ticket));
}
...
void EnsureActionLine(int ticket, string activeSuffix, string priceSuffix, string lineName, double brokerPrice, color orderColor, int style)
{
    ...
    UpdateLineLabel(ticket, priceSuffix, price, orderColor);
}
...
void DeleteLine(int ticket, string suffix)
{
    ...
    ObjectDelete(0, LineLabelName(ticket, suffix));
}
...
{
    return LineName(ticket, suffix) + "_LBL";
}

void UpdateLineLabel(int ticket, string suffix, double price, color lineColor)
{
    string labelName = LineLabelName(ticket, suffix);
    datetime firstVisibleTime = Time[WindowFirstVisibleBar()];
    string labelText = suffix + " #" + IntegerToString(ticket);

    if(ObjectFind(0, labelName) < 0)
    {
        ObjectCreate(0, labelName, OBJ_TEXT, 0, firstVisibleTime, price);
    ...
    ObjectSetInteger(0, labelName, OBJPROP_TIME, firstVisibleTime);
    ObjectSetString(0, labelName, OBJPROP_TEXT, labelText);
}

void CleanupStaleObjects(int &tickets[], int total)
{
    int totalObjects = ObjectsTotal(0, 0, -1);
    string prefix = "CC_" + Symbol() + "_";
    ...
    for(int i = totalObjects - 1; i >= 0; i--)
    {
        ...
        int ticketStart = StringLen(prefix);
        int suffixPos = StringFind(name, "_", ticketStart);
        if(suffixPos < 0)
            continue;

        string ticketStr = StringSubstr(name, ticketStart, suffixPos - ticketStart);
        int ticket = (int)StringToInteger(ticketStr);

        if(!IsTicketActive(ticket, tickets, total))
        {
            ObjectDelete(0, name);
            ClearTicketGlobals(ticket);
        }
    }
}

bool IsTicketActive(int ticket, int &tickets[], int total)
{
    for(int i = 0; i < total; i++)
    {
        if(tickets[i] == ticket)
...
void ClearTicketGlobals(int ticket)
{
    GlobalVariableDel(ControlKey(ticket, "SL"));
    GlobalVariableDel(ControlKey(ticket, "TP"));
    GlobalVariableDel(ControlKey(ticket, "PL1"));
    GlobalVariableDel(ControlKey(ticket, "PL2"));
    GlobalVariableDel(ControlKey(ticket, "BE"));
    GlobalVariableDel(ControlKey(ticket, "SLActive"));
    GlobalVariableDel(ControlKey(ticket, "TPActive"));
    GlobalVariableDel(ControlKey(ticket, "P1Active"));
    GlobalVariableDel(ControlKey(ticket, "P2Active"));
    GlobalVariableDel(ControlKey(ticket, "BELActive"));
    GlobalVariableDel(ControlKey(ticket, "PL1Pct"));
    GlobalVariableDel(ControlKey(ticket, "PL2Pct"));
    GlobalVariableDel(ControlKey(ticket, "TPWarned"));
}
